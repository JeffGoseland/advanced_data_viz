...
rm(list = ls())
# active_file <- "data/AI_ML_post2023.rData"
...
if (file.exists(active_file)){
  ...
  # filter(str_detect(categories, "cs.LG")) %>% # Find 'cs.LG' (Machine Learning)
  # 'created' is a better field than 'update_date'
...
taking just the ones since 2023
...
write_csv(author_topic_edges, "author_topic_edges.csv")
write_csv(author_topic_nodes, "author_topic_nodes.csv")
...
write_csv(topic_edges, "topic_topic_edges.csv")
write_csv(topic_nodes, "topic_topic_nodes.csv")
...
write_csv(author_author_edges, "author_author_edges.csv")
write_csv(author_nodes, "author_author_nodes.csv")
...
```{r}
g <- tbl_graph(nodes = author_topic_nodes, edges = author_topic_edges, directed = FALSE)
...
# Write the final CSV
write_csv(final_table_for_tableau, "arxiv_author_metrics.csv")
write_csv(final_table_for_tableau, "arxiv_nodes_final.csv")
